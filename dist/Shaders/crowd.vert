...
// invBindPose is like a matrix uniform array with regular skinning
uniform mat4 invBindPose[MAX_BONES];
// since the animated pose is not like invBindPose, the shader needs to sample the animTex
...
// Instead of model matrix, we have three uniform arrays (1 per component)
...
mat4 GetPose(int joint, int instance) {
    ...
    // using texelFetch to get RAW data
