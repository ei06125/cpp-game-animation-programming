...
Shader::Shader() { mHandle = glCreateProgram(); }
...
Shader::~Shader() { glDeleteProgram(mHandle); }
...
void Shader::PopulateUniforms() {
	...
	for (int i = 0; i < count; ++i) {
		memset(name, 0, sizeof(char) * 128);
		glGetActiveUniform(mHandle, (GLuint)i, 128, &length, &size, &type,
						   name);
			...
			if (found != std::string::npos) {
				...
				while (true) {
					...
					sprintf(testName, "%s[%d]", uniformName.c_str(),
							uniformIndex++);
					int uniformLocation =
						glGetUniformLocation(mHandle, testName);
...
void Shader::Bind() { glUseProgram(mHandle); }

void Shader::UnBind() { glUseProgram(0); }

unsigned int Shader::GetHandle() { return mHandle; }
