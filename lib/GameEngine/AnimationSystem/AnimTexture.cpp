...
void AnimTexture::UploadTextureDataToGPU() {
	...
	glTexImage2D(GL_TEXTURE_2D, 0, GL_RGBA32F, mSize, mSize, 0, GL_RGBA,
				 GL_FLOAT, mData);
...
unsigned int AnimTexture::Size() { return mSize; }

unsigned int AnimTexture::GetHandle() { return mHandle; }

float *AnimTexture::GetData() { return mData; }
...
vec4 AnimTexture::GetTexel(unsigned int x, unsigned int y) {
	...
	return vec4(mData[index + 0], mData[index + 1], mData[index + 2],
				mData[index + 3]);
}
