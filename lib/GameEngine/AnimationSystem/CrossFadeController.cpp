...
void CrossFadeController::FadeTo(Clip *target, float fadeTime) {
	...
	mTargets.push_back(
		CrossFadeTarget(target, mSkeleton.GetRestPose(), fadeTime));
}
...
void CrossFadeController::Update(float dt) {
	...
	for (unsigned int i = 0; i < numTargets; ++i) {
		/*
			Set the current animation as the target animation and
			remove the fade object if an animation has finished fading
		*/
	...
	/*
		...
		All the animations in the fade list will need to be sampled
	*/
...
Pose &CrossFadeController::GetCurrentPose() { return mPose; }

Clip *CrossFadeController::GetcurrentClip() { return mClip; }
