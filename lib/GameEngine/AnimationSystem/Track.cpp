...
template <typename T, int N> Track<T, N>::Track() {
	mInterpolation = Interpolation::Linear;
}

template <typename T, int N> float Track<T, N>::GetStartTime() {
	return mFrames[0].mTime;
}

template <typename T, int N> float Track<T, N>::GetEndTime() {
...
template <typename T, int N> void Track<T, N>::Resize(unsigned int size) {
	mFrames.resize(size);
}

template <typename T, int N> unsigned int Track<T, N>::Size() {
	return mFrames.size();
}

template <typename T, int N> Interpolation Track<T, N>::GetInterpolation() {
...
int FastTrack<T, N>::FrameIndex(float time, bool looping) {
	...
	// Make sure the track is valid (two or more frames)
	unsigned int size = (unsigned int)frames.size();
	if (size <= 1) {
		return -1;
	}

	// Make sure the requested sampled time falls between the start and end
	// times of the track
	if (looping) {
	...
	// The frame index is the normalized sample time scaled by the number of
	// samples
...
// Intended to be called at load time
// TODO optimize to be faster by remembering the last used index of the inner j
// loop,
// ... since, on each i iteration, the frame index only increases
template <typename T, int N> void FastTrack<T, N>::UpdateIndexLookupTable() {
	...
	// Find the number of samples needed
	float duration = this->GetEndTime() - this->GetStartTime();
	unsigned int numSamples = 60 + (unsigned int)(duration * 60.0f);
	mSampledFrames.resize(numSamples);

	for (unsigned int i = 0; i < numSamples; ++i) {
		// time = normalized iterator * duration + start time
		float t = (float)i / (float)(numSamples - 1);
		float time = t * duration + this->GetStartTime();

		unsigned int frameIndex = 0;
		for (int j = numFrames - 1; j >= 0; --j) {
			// Find the frame that comes before the sampling
			if (time >= this->mFrames[j].mTime) {
				// frame index should never return the last frame
...
// Intended to be called at load time
template <typename T, int N> FastTrack<T, N> OptimizeTrack(Track<T, N> &input) {
