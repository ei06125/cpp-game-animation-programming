...
namespace GLTFHelpers {
	Transform GetLocalTransform(cgltf_node &node) {
		...
		if (node.has_matrix) {
			...
			// return matrix?
			// the book says: "otherwise, load components as needed"
		}

		if (node.has_translation) {
			result.position = vec3(node.translation[0], node.translation[1],
								   node.translation[2]);
		}

		if (node.has_rotation) {
			result.rotation = quat(node.rotation[0], node.rotation[1],
	...
	int GetNodeIndex(cgltf_node *target, cgltf_node *allNodes,
	...
	void TrackFromChannel(Track<T, N> &inOutTrack,
						  const cgltf_animation_channel &inChannel) {
		cgltf_animation_sampler &sampler = *inChannel.sampler;

		Interpolation interpolation = Interpolation::Constant;
		if (inChannel.sampler->interpolation ==
			cgltf_interpolation_type_linear) {
			interpolation = Interpolation::Linear;
		} else if (inChannel.sampler->interpolation ==
		...
		unsigned int numberOfValuesPerFrame =
			valueFloats.size() / timelineFloats.size();
		...
		for (unsigned int i = 0; i < numFrames; ++i) {
			...
			for (int component = 0; component < N; ++component) {
				frame.mIn[component] =
					isSamplerCubic ? valueFloats[baseIndex + offset++] : 0.0f;
			}
			...
			for (int component = 0; component < N; ++component) {
				frame.mOut[component] =
					isSamplerCubic ? valueFloats[baseIndex + offset++] : 0.0f;
			}
		}
	}

	void MeshFromAttribute(Mesh &outMesh, cgltf_attribute &attribute,
						   cgltf_skin *skin, cgltf_node *nodes,
						   unsigned int nodeCount) {
		// Start by figuring out how many attributes the current component has
		...
		for (unsigned int i = 0; i < acessorCount; ++i) {
			...
			case cgltf_attribute_type_position:
				positions.push_back(vec3(values[index + 0], values[index + 1],
										 values[index + 2]));
				break;
			...
			case cgltf_attribute_type_weights:
				weights.push_back(vec4(values[index + 0], values[index + 1],
									   values[index + 2], values[index + 3]));
				break;
			case cgltf_attribute_type_normal: {
				vec3 normal = vec3(values[index + 0], values[index + 1],
								   values[index + 2]);
			...
			case cgltf_attribute_type_joints: {
				...
				// Make the joint indices relative to the skeleton instead of
				// the joints array Make sure there are no negative joint
				// indices (would break implementation)
				joints.x = std::max(
					0, GetNodeIndex(skin->joints[joints.x], nodes, nodeCount));
				joints.y = std::max(
					0, GetNodeIndex(skin->joints[joints.y], nodes, nodeCount));
				joints.z = std::max(
					0, GetNodeIndex(skin->joints[joints.z], nodes, nodeCount));
				joints.w = std::max(
					0, GetNodeIndex(skin->joints[joints.w], nodes, nodeCount));
...
Pose LoadRestPose(cgltf_data *data) {
	...
	for (unsigned int i = 0; i < boneCount; ++i) {
		...
		int parent =
			GLTFHelpers::GetNodeIndex(node->parent, data->nodes, boneCount);
...
std::vector<std::string> LoadJointNames(cgltf_data *data) {
	...
	// TODO: update skinned.vert Shader :: size of uniform mat4 to new boneCount
...
std::vector<Clip> LoadAnimationClips(cgltf_data *data) {
	...
	for (unsigned int i = 0; i < numClips; ++i) {
		...
		unsigned int numChannels =
			(unsigned int)data->animations[i].channels_count;
		for (unsigned int j = 0; j < numChannels; ++j) {
			cgltf_animation_channel &channel = data->animations[i].channels[j];
			cgltf_node *target = channel.target_node;
			int nodeId =
				GLTFHelpers::GetNodeIndex(target, data->nodes, numNodes);
...
Pose LoadBindPose(cgltf_data *data) {
	// Populate worldBindPose with the global transform of each joint in the
	// rest pose
	...
	for (unsigned int i = 0; i < numSkins; ++i) {
		...
		for (unsigned int j = 0; j < numJoints; ++j) {
			...
			int jointIndex =
				GLTFHelpers::GetNodeIndex(jointNode, data->nodes, numBones);
...
Skeleton LoadSkeleton(cgltf_data *data) {
	return Skeleton(LoadRestPose(data), LoadBindPose(data),
					LoadJointNames(data));
}
	...
	for (unsigned int i = 0; i < nodeCount; ++i) {
		...
		for (unsigned int j = 0; j < numPrims; ++j) {
			...
			for (unsigned int k = 0; k < numAttributes; ++k) {
				...
				GLTFHelpers::MeshFromAttribute(mesh, *attribute, node->skin,
											   nodes, nodeCount);
			}

			// If the primitive contains indices, the index buffer needs to be
			// filled
			if (primitive->indices != 0) {
				...
				for (unsigned int k = 0; k < indexCount; ++k) {
					indices[k] = (unsigned int)cgltf_accessor_read_index(
						primitive->indices, k);
				}
			}
