...
Pose::Pose(unsigned int numJoints) { Resize(numJoints); }
...
unsigned int Pose::Size() { return mJoints.size(); }

Transform Pose::GetLocalTransform(unsigned int index) { return mJoints[index]; }
...
int Pose::GetParent(unsigned int index) { return mParents[index]; }
