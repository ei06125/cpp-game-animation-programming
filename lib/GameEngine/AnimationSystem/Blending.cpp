...
void Blend(Pose &output, Pose &a, Pose &b, float t, int blendroot) {
	...
	for (unsigned int i = 0; i < numJoints; ++i) {
		...
		output.SetLocalTransform(
			i, mix(a.GetLocalTransform(i), b.GetLocalTransform(i), t));
	}
}
	...
	for (unsigned int i = 0; i < numJoints; ++i) {
		...
		Transform result(
			...
			/*
			   ...
			   To remove the rotation, we multiply by the inverse,
			   ... which results in the identity matrix.
			*/
			normalized(input.rotation *
					   (inverse(additiveBase.rotation) * additive.rotation)),
