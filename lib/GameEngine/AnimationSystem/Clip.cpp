...
template <typename TRACK> TClip<TRACK>::TClip() {
	mName = "No name given";
	mStartTime = 0.0f;
	mEndTime = 0.0f;
	mLooping = true;
...
template <typename TRACK> void TClip<TRACK>::RecalculateDuration() {
	mStartTime = 0.0f;
	mEndTime = 0.0f;
	bool startSet = false;
	bool endSet = false;
...
template <typename TRACK> TRACK &TClip<TRACK>::operator[](unsigned int joint) {
	for (unsigned int i = 0, size = (unsigned int)mTracks.size(); i < size;
		 ++i) {
...
template <typename TRACK> std::string &TClip<TRACK>::GetName() { return mName; }
...
template <typename TRACK> unsigned int TClip<TRACK>::Size() {
	return (unsigned int)mTracks.size();
}

template <typename TRACK> float TClip<TRACK>::GetDuration() {
	return mEndTime - mStartTime;
}

template <typename TRACK> float TClip<TRACK>::GetStartTime() {
	return mStartTime;
}

template <typename TRACK> float TClip<TRACK>::GetEndTime() { return mEndTime; }

template <typename TRACK> bool TClip<TRACK>::GetLooping() { return mLooping; }

template <typename TRACK> void TClip<TRACK>::SetLooping(bool inLooping) {
	mLooping = inLooping;
}
