...
struct mat4 {
	union {
		float v[16];
		// alias for basis vector stored in columns
		struct {
			vec4 right;
			vec4 up;
			vec4 forward;
		...
		// alias for pair oof basis vector and compoment of that vector
		struct {
