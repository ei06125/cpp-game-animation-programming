...
void Chapter12Sample01::Update(float deltaTime) {
	mAniamtionA.mPlayback = mClips[mAniamtionA.mClip].Sample(
		mAniamtionA.mPose, mAniamtionA.mPlayback + deltaTime);
	mAniamtionA.mVisual->FromPose(mAniamtionA.mPose);
	mAniamtionB.mPlayback = mClips[mAniamtionB.mClip].Sample(
		mAniamtionB.mPose, mAniamtionB.mPlayback + deltaTime);
	mAniamtionB.mVisual->FromPose(mAniamtionB.mPose);

	int blendRoot = (int)mBlendRoot - 1;
	mBlendedPose = mAniamtionA.mPose;
	Blend(mBlendedPose, mAniamtionA.mPose, mAniamtionB.mPose, mBlendFactor,
		  blendRoot);
	mCurrentPoseVisual->FromPose(mBlendedPose);

	if (mShowMesh) {
		if (mSkinType == SkinningType::CPU) {
			for (unsigned int i = 0, size = (unsigned int)mMeshes.size();
				 i < size; ++i) {
				SkinOnCPU(mSkeleton, mBlendedPose, mMeshes[i]);
	...
	if (mShowMesh) {
		...
		for (unsigned int i = 0, size = (unsigned int)mMeshes.size(); i < size;
			...
			mMeshes[i].Bind(shader->GetAttribute("position"),
							...
							shader->GetAttribute("texCoord"), weights,
							influences);
			mMeshes[i].Draw();
...
void Chapter12Sample01::ImGui(nk_context *ctx) {
	nk_begin(ctx, "Chapter 9, Sample 1", nk_rect(5.0f, 5.0f, 300.0f, 355.0f),
	...
	int selected = nk_combo(ctx, (const char **)mUIClipNames, mNumUIClips,
							(int)mAniamtionA.mClip, 25, nk_vec2(200, 200));
	...
	selected = nk_combo(ctx, (const char **)mUIClipNames, mNumUIClips,
						(int)mAniamtionB.mClip, 25, nk_vec2(200, 200));
	...
	mBlendRoot =
		(unsigned int)nk_combo(ctx, (const char **)mBoneNames, mNumBoneNames,
							   (int)mBlendRoot, 25, nk_vec2(200, 200));
	...
	selected = nk_combo(ctx, (const char **)mSkinningNames, 3, (int)skinIndex,
						25, nk_vec2(200, 200));
