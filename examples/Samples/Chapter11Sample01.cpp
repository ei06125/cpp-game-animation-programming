...
void Chapter11Sample01::Update(float deltaTime) {
  ...
  for (unsigned int i = 0, size = (unsigned int)mPosePalette.size(); i < size;
       ++i) {
  ...
  if (mSkinType == SkinningType::CPU) {
    for (unsigned int i = 0, size = (unsigned int)mMeshes.size(); i < size;
         ++i) {
      SkinOnCPU(mPosePalette, mMeshes[i]);
...
void Chapter11Sample01::Render(float inAspectRatio) {
  ...
  for (unsigned int i = 0, size = (unsigned int)mMeshes.size(); i < size; ++i) {
    if (mSkinType == SkinningType::GPU) {
      mMeshes[i].Bind(
          shader->GetAttribute("position"), shader->GetAttribute("normal"),
          shader->GetAttribute("texCoord"), shader->GetAttribute("weights"),
      ...
      mMeshes[i].Bind(shader->GetAttribute("position"),
                      ...
                      shader->GetAttribute("texCoord"), -1, -1);
      mMeshes[i].Draw();
...
void Chapter11Sample01::ImGui(nk_context *ctx) {
  nk_begin(ctx, "Chapter 9, Sample 1", nk_rect(5.0f, 5.0f, 300.0f, 140.0f),
