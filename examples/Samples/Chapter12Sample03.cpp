...
void Chapter12Sample03::Update(float deltaTime) {
  ...
  float time = mClips[mAdditiveIndex].GetStartTime() +
               (mClips[mAdditiveIndex].GetDuration() * mAdditiveTime);
  ...
  if (mSkinType == SkinningType::CPU) {
    for (unsigned int i = 0, size = (unsigned int)mMeshes.size(); i < size;
         ++i) {
      SkinOnCPU(mSkeleton, mCurrentPose, mMeshes[i]);
...
void Chapter12Sample03::ImGui(nk_context *ctx) {
  nk_begin(ctx, "Chapter 9, Sample 1", nk_rect(5.0f, 5.0f, 300.0f, 225.0f),
  ...
  int selected = nk_combo(ctx, (const char **)mUIClipNames, mNumUIClips,
                          (int)mCurrentClip, 25, nk_vec2(200, 200));
  ...
  selected = nk_combo(ctx, (const char **)mUIClipNames, mNumUIClips,
                      (int)mAdditiveIndex, 25, nk_vec2(200, 200));
