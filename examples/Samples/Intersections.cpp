...
std::vector<Triangle> MeshToTriangles(Mesh &mesh)
{
    ...
    if (indices.size() == 0)
    {
        ...
        {
            result.push_back(Triangle(vertices[i + 0], vertices[i + 1], vertices[i + 2]));
        }
    }
    else
    {
        ...
        {
            result.push_back(Triangle(vertices[indices[i + 0]], vertices[indices[i + 1]], vertices[indices[i + 2]]));
        }
    }
    return result;
}
...
std::vector<Triangle> MeshesToTriangles(std::vector<Mesh> &meshes)
{
    ...
    for (unsigned int j = 0; j < numMeshes; ++j)
    {
        ...
        else
        {
            ...
            {
                result.push_back(
                    Triangle(vertices[indices[i + 0]], vertices[indices[i + 1]], vertices[indices[i + 2]]));
            }
        }
    }
    return result;
