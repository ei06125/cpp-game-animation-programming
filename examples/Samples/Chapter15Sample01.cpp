...
void Chapter15Sample01::SetCrowdSize(unsigned int size) {
	...
	for (unsigned int i = 0; i < numCrowds; ++i) {
		...
		mCrowds[i].RandomizePositions(occupied, vec3(-40, 0, -80.0f),
									  vec3(40, 0, 30.0f), 2.0f);
	}
}
...
void Chapter15Sample01::Render(float inAspectRatio) {
	...
	Uniform<mat4>::Set(mCrowdShader->GetUniform("invBindPose"),
	...
	for (unsigned int c = 0; c < numCrowds; ++c) {
		...
		for (unsigned int i = 0, size = (unsigned int)mMeshes.size(); i < size;
			 ++i) {
			mMeshes[i].Bind(mCrowdShader->GetAttribute("position"),
							mCrowdShader->GetAttribute("normal"),
							mCrowdShader->GetAttribute("texCoord"),
							mCrowdShader->GetAttribute("weights"),
							mCrowdShader->GetAttribute("joints"));
			mMeshes[i].DrawInstanced(mCrowds[c].Size());
			mMeshes[i].UnBind(mCrowdShader->GetAttribute("position"),
							  mCrowdShader->GetAttribute("normal"),
							  mCrowdShader->GetAttribute("texCoord"),
							  mCrowdShader->GetAttribute("weights"),
							  mCrowdShader->GetAttribute("joints"));
