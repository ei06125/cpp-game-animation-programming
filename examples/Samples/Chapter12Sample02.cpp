	...
	if (mShowMesh) {
		if (mSkinType == SkinningType::CPU) {
			for (unsigned int i = 0, size = (unsigned int)mMeshes.size();
				 i < size; ++i) {
				SkinOnCPU(mSkeleton, mController.GetCurrentPose(), mMeshes[i]);
...
void Chapter12Sample02::ImGui(nk_context *ctx) {
	nk_begin(ctx, "Chapter 9, Sample 1", nk_rect(5.0f, 5.0f, 300.0f, 220.0f),
	...
	int selected = nk_combo(ctx, (const char **)mUIClipNames, mNumUIClips,
							(int)mNextAnim, 25, nk_vec2(200, 200));
