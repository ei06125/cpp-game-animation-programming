...
#include "AnimationSystem/RearrangeBones.h"
...
void Chapter11Sample03::Render(float inAspectRatio)
{
    ...
    for (unsigned int i = 0, size = (unsigned int)mMeshes.size(); i < size; ++i)
    {
        ...
        mMeshes[i].Bind(shader->GetAttribute("position"), shader->GetAttribute("normal"),
                        shader->GetAttribute("texCoord"), weights, influences);
        mMeshes[i].Draw();
        mMeshes[i].UnBind(shader->GetAttribute("position"), shader->GetAttribute("normal"),
                          shader->GetAttribute("texCoord"), weights, influences);
    }
