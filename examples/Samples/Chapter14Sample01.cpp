...
void Chapter14Sample01::Render(float inAspectRatio) {
	...
	if (mSkinningMethod == 0) {
		Uniform<DualQuaternion>::Set(shader->GetUniform("pose"),
									 mDqPosePalette);
		Uniform<DualQuaternion>::Set(shader->GetUniform("invBindPose"),
									 mDqInvBindPalette);
	} else {
...
void Chapter14Sample01::ImGui(nk_context *ctx) {
	nk_begin(ctx, "Chapter 9, Sample 1", nk_rect(5.0f, 5.0f, 300.0f, 40.0f),
	...
	mSkinningMethod = nk_combo(ctx, (const char **)mSkinningNames, 2,
							   mSkinningMethod, 25, nk_vec2(200, 200));
