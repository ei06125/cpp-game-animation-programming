...
#include "ThirdPartySDKs/glad/glad.h"
#include "AnimationSystem/GLTFLoader.h"
#include <iostream>
...
void Chapter13Sample03::Render(float inAspectRatio)
{
    ...
    mat4 view = lookAt(vec3(mModel.position.x, 0, mModel.position.z) + vec3(0, 5, 10),
                       vec3(mModel.position.x, 0, mModel.position.z) + vec3(0, 3, 0), vec3(0, 1, 0));
    ...
    if (mShowMesh)
    {
        ...
        {
            mMeshes[i].Bind(characterShader->GetAttribute("position"), characterShader->GetAttribute("normal"),
                            characterShader->GetAttribute("texCoord"), characterShader->GetAttribute("weights"),
                            characterShader->GetAttribute("joints"));
            mMeshes[i].Draw();
            mMeshes[i].UnBind(characterShader->GetAttribute("position"), characterShader->GetAttribute("normal"),
                              characterShader->GetAttribute("texCoord"), characterShader->GetAttribute("weights"),
                              characterShader->GetAttribute("joints"));
        }
    ...
    if (mShowEnvironment)
    {
        ...
        {
            mIKCourse[i].Bind(environmentShader->GetAttribute("position"), environmentShader->GetAttribute("normal"),
                              environmentShader->GetAttribute("texCoord"), -1, -1);
            mIKCourse[i].Draw();
            mIKCourse[i].UnBind(environmentShader->GetAttribute("position"), environmentShader->GetAttribute("normal"),
                                environmentShader->GetAttribute("texCoord"), -1, -1);
        }
