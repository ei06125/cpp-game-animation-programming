...
void Chapter13Sample03::Initialize() {
	...
	mMotionTrack[0].mValue[0] = 0;
	mMotionTrack[0].mValue[2] = 1;
	mMotionTrack[1].mTime = 1.0f;
	mMotionTrack[1].mValue[0] = 0;
	mMotionTrack[1].mValue[2] = 10;
	mMotionTrack[2].mTime = 3.0f;
	mMotionTrack[2].mValue[0] = 22;
	mMotionTrack[2].mValue[2] = 10;
	mMotionTrack[3].mTime = 4.0f;
	mMotionTrack[3].mValue[0] = 22;
	mMotionTrack[3].mValue[2] = 2;
	mMotionTrack[4].mTime = 6.0f;
	mMotionTrack[4].mValue[0] = 0;
	mMotionTrack[4].mValue[2] = 1;
	...
	mLeftLeg =
		new IKLeg(mSkeleton, "LeftUpLeg", "LeftLeg", "LeftFoot", "LeftToeBase");
	mLeftLeg->SetAnkleOffset(0.2f);
	mRightLeg = new IKLeg(mSkeleton, "RightUpLeg", "RightLeg", "RightFoot",
						  "RightToeBase");
	...
	// For the UI
	for (unsigned int i = 0, size = (unsigned int)mClips.size(); i < size;
		 ++i) {
	...
	// Start the character clamped to the ground. Move down a little bit so it's
	// not perfectly up
...
void Chapter13Sample03::Update(float deltaTime) {
	...
	// Increment time and sample the animation clip that moves the model on the
	// level rails The Y position is a lie, it's a trackt hat only makes sense
	// from an ortho top view
	mWalkingTime += deltaTime * 0.3f;
	while (mWalkingTime > 6.0f) {
		mWalkingTime -= 6.0f;
	}

	// Figure out the X and Z position of the model in world spcae
	// Keep the Y position the same as last frame for both to properly orient
	// the model
	...
	for (unsigned int i = 0; i < numTriangles; ++i) {
		if (RaycastTriangle(groundRay, mTriangles[i], hitPoint)) {
			// Sink the model a little bit into the ground to avoid hyper
			// extending it's legs
	...
	// Sample the current animation, update the pose visual and figure out where
	// the left and right leg are in their up/down animation cycle
	mPlaybackTime =
		mClips[mCurrentClip].Sample(mCurrentPose, mPlaybackTime + deltaTime);
	mCurrentPoseVisual->FromPose(mCurrentPose);
	float normalizedTime =
		(mPlaybackTime - mClips[mCurrentClip].GetStartTime()) /
	...
	// Construct a ray for the left ankle, store the world position and the
	// predictive position of the ankle. This is in case the raycasts below
	// don't hit anything.
	vec3 worldLeftAnkle =
		combine(mModel, mCurrentPose.GetGlobalTransform(mLeftLeg->Ankle()))
			.position;
	Ray leftAnkleRay(worldLeftAnkle + vec3(0, 2, 0));
	vec3 predictiveLeftAnkle = worldLeftAnkle;

	// Construct a ray for the right ankle, store the world position and the
	// predictive position of the ankle. This is in case the raycasts below
	// don't hit anything.
	vec3 worldRightAnkle =
		combine(mModel, mCurrentPose.GetGlobalTransform(mRightLeg->Ankle()))
			.position;
	...
	// the current position (raycast from knee height to the sole of the foot
	// height) and the predictive position (infinate ray cast). The target point
	// will be between these two goals
	...
	for (unsigned int i = 0; i < numTriangles; ++i) {
		...
		if (RaycastTriangle(rightAnkleRay, mTriangles[i], hitPoint)) {
			if (lenSq(hitPoint - rightAnkleRay.origin) <
				rayHeight * rayHeight) {
	...
	// Lerp between fully clamped to the ground, and somewhat clamped to the
	// ground based on the current phase of the walk cycle
	...
	// Now that we know the position of the model, as well as the ankle we can
	// solve the feet.
	mLeftLeg->SolveForLeg(mModel, mCurrentPose,
						  worldLeftAnkle /*, worldLeftToe*/);
	mRightLeg->SolveForLeg(mModel, mCurrentPose,
						   worldRightAnkle /*, worldRightToe*/);
	// Apply the solved feet
	Blend(mCurrentPose, mCurrentPose, mLeftLeg->GetAdjustedPose(), 1,
		  mLeftLeg->Hip());
	Blend(mCurrentPose, mCurrentPose, mRightLeg->GetAdjustedPose(), 1,
		  mRightLeg->Hip());

	// The toes are still wrong, let's fix those. First, construct some rays for
	// the toes
	...
	vec3 worldLeftToe =
		combine(mModel, mCurrentPose.GetGlobalTransform(mLeftLeg->Toe()))
			.position;
	vec3 leftToeTarget = worldLeftToe;
	vec3 predictiveLeftToe = worldLeftToe;

	vec3 worldRightToe =
		combine(mModel, mCurrentPose.GetGlobalTransform(mRightLeg->Toe()))
			.position;
	...
	if (dot(leftAnkleToCurrentToe, leftAnkleToDesiredToe) > 0.00001f) {
		quat ankleRotator =
			fromTo(leftAnkleToCurrentToe, leftAnkleToDesiredToe);
		Transform ankleLocal =
			mCurrentPose.GetLocalTransform(mLeftLeg->Ankle());

		quat worldRotatedAnkle = leftAnkleWorld.rotation * ankleRotator;
		quat localRotatedAnkle =
			worldRotatedAnkle * inverse(leftAnkleWorld.rotation);
	...
	if (dot(rightAnkleToCurrentToe, rightAnkleToDesiredToe) > 0.00001f) {
		quat ankleRotator =
			fromTo(rightAnkleToCurrentToe, rightAnkleToDesiredToe);
		Transform ankleLocal =
			mCurrentPose.GetLocalTransform(mRightLeg->Ankle());

		quat worldRotatedAnkle = rightAnkleWorld.rotation * ankleRotator;
		quat localRotatedAnkle =
			worldRotatedAnkle * inverse(rightAnkleWorld.rotation);
...
void Chapter13Sample03::Render(float inAspectRatio) {
	...
	mat4 view =
		lookAt(vec3(mModel.position.x, 0, mModel.position.z) + vec3(0, 5, 10),
	...
	if (mShowMesh) {
		...
		Uniform<mat4>::Set(characterShader->GetUniform("projection"),
						   projection);
		...
		Uniform<mat4>::Set(characterShader->GetUniform("invBindPose"),
						   mSkeleton.GetInvBindPose());

		mDiffuseTexture->Set(characterShader->GetUniform("tex0"), 0);
		for (unsigned int i = 0, size = (unsigned int)mMeshes.size(); i < size;
			 ++i) {
			mMeshes[i].Bind(characterShader->GetAttribute("position"),
			...
			mMeshes[i].Draw();
			mMeshes[i].UnBind(characterShader->GetAttribute("position"),
							  characterShader->GetAttribute("normal"),
							  characterShader->GetAttribute("texCoord"),
							  characterShader->GetAttribute("weights"),
							  characterShader->GetAttribute("joints"));
	...
	if (mShowEnvironment) {
		...
		Uniform<mat4>::Set(environmentShader->GetUniform("projection"),
						   projection);
		Uniform<vec3>::Set(environmentShader->GetUniform("light"),
						   vec3(1, 1, 1));
		mCourseTexture->Set(environmentShader->GetUniform("tex0"), 0);
		for (unsigned int i = 0, size = (unsigned int)mIKCourse.size();
			 i < size; ++i) {
			mIKCourse[i].Bind(environmentShader->GetAttribute("position"),
							  environmentShader->GetAttribute("normal"),
							  environmentShader->GetAttribute("texCoord"), -1,
							  -1);
			mIKCourse[i].Draw();
			mIKCourse[i].UnBind(environmentShader->GetAttribute("position"),
								environmentShader->GetAttribute("normal"),
								environmentShader->GetAttribute("texCoord"), -1,
								-1);
		}
...
void Chapter13Sample03::ImGui(nk_context *ctx) {
	nk_begin(ctx, "Chapter 9, Sample 1", nk_rect(5.0f, 5.0f, 300.0f, 130.0f),
