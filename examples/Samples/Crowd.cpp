...
unsigned int Crowd::Size() { return mCurrentPlayTimes.size(); }
...
void Crowd::UpdatePlaybackTimes(float deltaTime, bool looping, float start,
								float end) {
...
// will convert current play time into pixel coordinates along the animation
// texture's x axis
void Crowd::UpdateFrameIndices(float start, float duration,
							   unsigned int texWidth) {
	unsigned int size = mCurrentPlayTimes.size();
	for (unsigned int i = 0; i < size; ++i) {
		float thisNormalizedTime = (mCurrentPlayTimes[i] - start) / duration;
		unsigned int thisFrame =
			(unsigned int)(thisNormalizedTime * (float)(texWidth - 1));
		float nextNormalizedTime = (mNextPlayTimes[i] - start) / duration;
		unsigned int nextFrame =
			(unsigned int)(nextNormalizedTime * (float)(texWidth - 1));
...
// should find the interpolation time between the current and next animated
// frames
void Crowd::UpdateInterpolationTimes(float start, float duration,
									 unsigned int texWidth) {
	unsigned int size = mCurrentPlayTimes.size();
	for (unsigned int i = 0; i < size; ++i) {
...
void Crowd::RandomizePositions(std::vector<vec3> &existing, const vec3 &min,
	...
	while (positions.size() < size) {
		...
		vec3 random((float)rand() / (float)RAND_MAX,
					(float)rand() / (float)RAND_MAX,
					(float)rand() / (float)RAND_MAX);
		vec3 point = min + delta * random;

		bool valid = true;
		for (unsigned int i = 0, iSize = (unsigned int)existing.size();
			 i < iSize; ++i) {
	...
	memcpy(mPositions[0].v, positions[0].v,
		   sizeof(float) * positions.size() * 3);
}
