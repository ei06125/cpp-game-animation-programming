...
/********************************************************************************************************************
 * The following definitions reduce the amount of code imported by windows.h
 *******************************************************************************************************************/
...
NK_API int nk_win32_handle_event(HWND hwnd, UINT msg, WPARAM wparam,
								 LPARAM lparam, float scaleFactor);
NK_API void nk_win32_render(int width, int height, int display_width,
							int display_height);
NK_API void nk_blank_render(int width, int height, int display_width,
							int display_height);
...
/********************************************************************************************************************
 * The following constants help create an OpenGL 3.3 Core context, as well as
 * define a function pointer type for wglCreateContextAttribsARB from wglext.h
 *******************************************************************************************************************/
...
/********************************************************************************************************************
 * The following type definitions are required to have vsynch enabled through
 * wglSwapIntervalEXT, which needs to be loaded together with
 * wglGetExtensionStringEXT and wglGetSwapIntervalEXT
 *******************************************************************************************************************/
...
/********************************************************************************************************************
 * To make logging a bit easier, two windows will be open in debug mode.
 * One will be the standard Win32 window, and the other will be a log console
 * We use pragmas instead of VS project properties to set this differences
 *******************************************************************************************************************/
#if _DEBUG
#include <crtdbg.h>
...
///*****************************************************************************
// * The reason these two variables exist globaly is
// * to easily reference them when the application is
// * shutting down later
// ****************************************************************************/
// Application *gApplication = 0;
// GLuint gVertexArrayObject = 0;
...
int WINAPI WinMain(HINSTANCE hInstance, HINSTANCE hPrevInstance, PSTR szCmdLine,
	...
	// Bind global pointer to a new instance
	// gApplication = new Application();

	// Configure a new Window Class
	WNDCLASSEX wndclass = {};
	...
	// Launch a new application in the center of the monitor
	...
	// To figure out the size of the window and not just the client area
	// the style of the window needs to be known
	DWORD style = (WS_OVERLAPPED | WS_CAPTION | WS_SYSMENU | WS_MINIMIZEBOX |
				   WS_MAXIMIZEBOX); // WS_THICKFRAME to resize

	// Adjust the size of the client rectangle to include all the window
	// dressing in its size as well
	...
	// ... Find the correct pixel format, and then apply it to the device
	...
	// ... create a temporary OpenGL context to get a pointer to a modern
	// context
	...
	// ... create a modern context from wglCreateContextAttribsARB
	const int attribList[] = {
	...
	// ... bind the new context and delete the temporary
	wglMakeCurrent(NULL, NULL);
	wglDeleteContext(tempRC);
	wglMakeCurrent(hdc, hglrc);

	// Initialize GLAD and load all the OpenGL 3.3 Core functions
	if (!gladLoadGL()) {
	...
	// Enable vsynch on the window
	// ... check if vsynch extensino string is in the list of extension strings
	...
	while (true) {
		...
		accumulator.win32Events +=
			(double)timerDiff * 1000.0 / (double)timerFrequency.QuadPart;
		...
		float deltaTime =
			float(thisTick - lastTick) * 0.001f; // 16.6 ms for 60fps
		lastTick = thisTick;
		...
		if (gSampleSelector.IsRunning()) {
			...
			if (!firstRenderSample) { // Application GPU Timer
				glGetQueryObjectiv(gGpuApplicationStop, GL_QUERY_RESULT,
								   &timerResultAvailable);
				while (!timerResultAvailable) {
					std::cout << "Waiting on app GPU timer!\n";
					glGetQueryObjectiv(gGpuApplicationStop, GL_QUERY_RESULT,
									   &timerResultAvailable);
				}
				glGetQueryObjectui64v(gGpuApplicationStart, GL_QUERY_RESULT,
									  &gpuStartTime);
				glGetQueryObjectui64v(gGpuApplicationStop, GL_QUERY_RESULT,
									  &gpuStopTime);
				accumulator.appGPU +=
					(double)(gpuStopTime - gpuStartTime) / 1000000.0;
			}
		...
		QueryPerformanceCounter(&timerStop);
		timerDiff = timerStop.QuadPart - timerStart.QuadPart;
		accumulator.frameRender +=
			(double)timerDiff * 1000.0 / (double)timerFrequency.QuadPart;

		// IMGUI Update
		QueryPerformanceCounter(&timerStart);
		if (gNkContext != 0 && gVertexArrayObject != 0) {
			float imguiXPosition =
				((float)clientWidth * gInvScaleFactor) - 205.0f;
			if (nk_begin(gNkContext, "Display Stats",
						 nk_rect(imguiXPosition, 5.0f, 200.0f, 65.0f),
						 NK_WINDOW_BORDER | NK_WINDOW_NO_SCROLLBAR)) {
				nk_layout_row_static(gNkContext, 15, 200, 1);

				sprintf(printBuffer, "Display frequency: %d\0",
						displayFrequency);
			...
			if (nk_begin(gNkContext, "High Level Timers",
						 nk_rect(imguiXPosition, 75.0f, 200.0f, 45.0f),
						 NK_WINDOW_BORDER | NK_WINDOW_NO_SCROLLBAR)) {
				nk_layout_row_static(gNkContext, 15, 200, 1);
				...
				sprintf(printBuffer, "Frame Time: %0.5f ms\0",
						display.frameTime);
				nk_label(gNkContext, printBuffer, NK_TEXT_LEFT);

				sprintf(printBuffer, "Delta Time: %0.5f ms\0",
						display.deltaTime);
				nk_label(gNkContext, printBuffer, NK_TEXT_LEFT);
			...
			if (nk_begin(gNkContext, "GPU Timers",
						 nk_rect(imguiXPosition, 125.0f, 200.0f, 45.0f),
						 NK_WINDOW_BORDER | NK_WINDOW_NO_SCROLLBAR)) {
				nk_layout_row_static(gNkContext, 15, 200, 1);
			...
			if (nk_begin(gNkContext, "CPU Timers",
						 nk_rect(imguiXPosition, 175.0f, 200.0f, 120.0f),
						 NK_WINDOW_BORDER | NK_WINDOW_NO_SCROLLBAR)) {
				nk_layout_row_static(gNkContext, 15, 200, 1);

				sprintf(printBuffer, "Win32 Events: %0.5f ms\0",
						display.win32Events);
				nk_label(gNkContext, printBuffer, NK_TEXT_LEFT);

				sprintf(printBuffer, "Game Update: %0.5f ms\0",
						display.frameUpdate);
				nk_label(gNkContext, printBuffer, NK_TEXT_LEFT);

				sprintf(printBuffer, "Game Render: %0.5f ms\0",
						display.frameRender);
				nk_label(gNkContext, printBuffer, NK_TEXT_LEFT);

				sprintf(printBuffer, "IMGUI logic: %0.5f ms\0",
						display.imguiLogic);
				nk_label(gNkContext, printBuffer, NK_TEXT_LEFT);

				sprintf(printBuffer, "IMGUI render: %0.5f ms\0",
						display.imguiRender);
				nk_label(gNkContext, printBuffer, NK_TEXT_LEFT);

				sprintf(printBuffer, "Swap Buffers: %0.5f ms\0",
						display.swapBuffer);
				nk_label(gNkContext, printBuffer, NK_TEXT_LEFT);
			}
			nk_end(gNkContext);
		...
		if (gNkContext != 0 && gVertexArrayObject != 0) {
			if (!firstRenderSample) { // Imgui GPU Timer
				glGetQueryObjectiv(gGpuImguiStop, GL_QUERY_RESULT,
								   &timerResultAvailable);
				while (!timerResultAvailable) {
				...
				glGetQueryObjectui64v(gGpuImguiStart, GL_QUERY_RESULT,
									  &gpuStartTime);
				glGetQueryObjectui64v(gGpuImguiStop, GL_QUERY_RESULT,
									  &gpuStopTime);
				accumulator.imguiGPU +=
					(double)(gpuStopTime - gpuStartTime) / 1000000.0;
			}

			glQueryCounter(gGpuImguiStart, GL_TIMESTAMP);
			nk_win32_render((int)((float)clientWidth * gInvScaleFactor),
							(int)((float)clientHeight * gInvScaleFactor),
							clientWidth, clientHeight);
			glQueryCounter(gGpuImguiStop, GL_TIMESTAMP);
		}
		QueryPerformanceCounter(&timerStop);
		timerDiff = timerStop.QuadPart - timerStart.QuadPart;
		accumulator.imguiRender +=
			(double)timerDiff * 1000.0 / (double)timerFrequency.QuadPart;
...
LRESULT CALLBACK WndProc(HWND hwnd, UINT iMsg, WPARAM wParam, LPARAM lParam) {
	...
	case WM_CLOSE:
		if (gSampleSelector.IsRunning()) {
			...
			std::cout << "Trying to close window after application has been "
						 "shut down!\n";
		}
		DestroyWindow(hwnd);
		break;
	...
	if (gNkContext != 0 && gVertexArrayObject != 0) {
		if (nk_win32_handle_event(hwnd, iMsg, wParam, lParam,
								  gInvScaleFactor)) {
			return 0;
		}
	}
